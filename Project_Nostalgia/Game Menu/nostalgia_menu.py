...
bg_image = pygame.image.load("Game Menu/Graphic/bg.png")

def get_font(size): # Returns Press-Start-2P in the desired size
    return pygame.font.Font("Game Menu/Graphic/font.ttf", size)

# Load the sound
pygame.mixer.music.load("Game Menu/Sound/bg.mp3")
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if flappy_bird_button.checkForInput(mouse_pos):
                    Popen(["python3", "Flappy Bird/flappy_bird.py"])
                    return
                if tetris_button.checkForInput(mouse_pos):
                    Popen(["python3", "Tetris/tetris_main.py"])
                    return
                if space_invader_button.checkForInput(mouse_pos):
                    Popen(["python3", "Space Invader/spa_inv_main.py"])
                    return
